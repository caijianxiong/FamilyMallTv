...
public class GetPdfImgAsyncTask extends AsyncTask<Void, Void, Throwable> {
    ...
    public GetPdfImgAsyncTask(String url, String index, OnBitmapCompleteListener listener) {
        this.index = index;
        this.url = url;
...
//            ParcelFileDescriptor pfd = mContext.getContentResolver().openFileDescriptor(getPdfUri(filePath), "r");
            ...
            if(pos<pdfiumCore.getPageCount(pdfDocument) && !cancelled){
...
//                render = cpy;
                ...
                logUtil.d_3(TAG,"获取图片成功，返回地址 ="+bitmapPath +" ("+pageWidth+","+pageHeight+")");
            }
            pfd.close();
            return null;
        } catch (Throwable t) {
