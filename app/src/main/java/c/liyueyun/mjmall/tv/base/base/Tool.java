...
//        }
     ...
     * 获取yun2win的图片
     */
    public static String getYun2winImg(String src){
        if(src == null){
            return src;
        }
        String yunSrc = "";
        if(src.contains("http://") || src.contains("https://")){
            if(src.contains(".yun2win.com") && UserManage.getInstance().getLocalUser()!=null){
                yunSrc = src+(src.contains("?")?"&":"?")+"access_token="+ UserManage.getInstance().getLocalUser().getLoginResult().getToken();
            }else {
                yunSrc = src;
            }
        }else {
            ...
            if(UserManage.getInstance().getLocalUser()!=null){
                yunSrc = MyConstant.DataUrl+"/v1"+yunSrc+(src.contains("?")?"&":"?")+"access_token="+ UserManage.getInstance().getLocalUser().getLoginResult().getToken();
            }else{
                yunSrc = MyConstant.DataUrl+"/v1"+yunSrc;
            }
        }
        return yunSrc;
    }
